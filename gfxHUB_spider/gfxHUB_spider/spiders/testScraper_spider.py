...
class TestScraperSpider(scrapy.Spider):
    ...
    def parse(self, response: Response):
        ...
        # for item in self.parse_cards_by_category(response):
        #     print('\n'*10)
        #     yield item
        #     print('\n'*10)
    ...
    def parse_page_cards_by_urls(self, response: Response):
        ...
        for item_block in response.xpath(item_block_xpath):
            # yield {
            #     'title': item_block.xpath(item_block_title_xpath).get().replace("\n", ""),
            #     'url': item_block.xpath(item_block_url_xpath).get()
            # }
            ...
            # _item["download_links"] = [f"https://site.net/download/{i}" for i in range(2)]

            # _posts = post_list_item["posts"]
            # print(_posts)
            # print(post_list_item["posts"])
            # _posts.append(_item)
        ...
        # self.data["data"]["category"].append(post_list_item["posts"])
